...
public class SysCompanyDao {
	...
	//删除
	public void deleteSysCompany(SysCompany sysCompa, Integer userId) throws Exception {
		SysUser sysuser =sysCompa.getUser();
		
		String hql = "delete SysCompany where companyId=?0";
		String hqll = "delete SysUser where userId=?0";
		dao.bulkUpdate(hql, sysCompa.getCompanyId());
		dao.bulkUpdate(hqll, userId);
	}
	//根据Id查询
